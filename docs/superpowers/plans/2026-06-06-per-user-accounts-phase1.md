...
- [ ] `User` (email unique, name, passwordHash, role owner|admin|member,
  isActive, lastLoginAt) + `UserSession` (tokenHash unique, userId Cascade,
  expiresAt) models.
- [ ] `user-auth.ts`: `usersExist()` (30s cache + invalidate), `createUser`,
...
- [ ] Tests: create/verify (wrong password, inactive user), session round-trip
  + expiry, bootstrap with and without a DB hash, cache invalidation.

## Task 2 — Session shim + login flow

- [ ] admin-session.ts keeps every export name: `hasAdminSession` resolves
...
- [ ] Session route: GET += `usersExist` + `user` {name,email,role}; POST
...
- [ ] `adminLoginSchema` += optional email; helpers/auth.ts factory += new
  exports.
- [ ] Tests: legacy login bootstraps owner, email login, wrong email 401,
  deactivated user 401, recovery mode.

## Task 3 — Users routes + role enforcement

- [ ] `GET/POST api/admin/users` + `PUT api/admin/users/[userId]` —
