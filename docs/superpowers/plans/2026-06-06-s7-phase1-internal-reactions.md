...
- [ ] Schema: `Reaction.dryRun Boolean @default(false)`; `Task.archivedAt DateTime?`
  + `@@index([projectId, archivedAt])`. Push + generate.
- [ ] Contracts: `reactionTypeSchema` += `task:assign`, `task:set-priority`,
  `task:set-retry`, `task:archive`, `step:escalate`; `createReactionSchema` += `dryRun`.
- [ ] `reactions/internal.ts`: the five executors, each (a) idempotent — clean
...
- [ ] Tests (`internal-reactions.test.ts`): assign happy/skip-if-assigned/role
...
- [ ] `executeReactions` passes `{projectId, taskId}` context to `dispatchReaction`;
...
- [ ] dispatch.ts: exhausted-retries step-failed payload += `mode`, `maxRetries`;
  failStep payload += `mode`.
- [ ] Board/task queries also exclude archived: tasks GET, project include,
  agent/next, agent/tasks, cli (mirror every `deletedAt: null` site).
- [ ] Archive surfaces mirroring deleted-tasks: `GET api/projects/[id]/archived-tasks`,
  `POST api/tasks/[id]/unarchive`.
- [ ] Tests: dryRun short-circuit, step-failed payload fields, archived-tasks routes.

## Task 3 — UI exposure + help + wrap-up

- [ ] settings-integrations.tsx: REACTION_TYPES grows an internal-actions group
  (labels prefixed "⚙ Internal:"), config placeholder hints per type, dryRun checkbox
  on the new-reaction form.
- [ ] Help: Settings · Automation section gains the shipped Phase 1 reality + recipe
  examples; roadmap callout updated to "Phase 1 shipped, sweep + UI forms next".
- [ ] Mark S7 roadmap entry "Phase 1 shipped"; full verification; commits per task.
